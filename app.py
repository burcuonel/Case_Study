    ...
    with col_stats:
        ...
        if selected_param in sensor_categories:
            ...
            if param_data.notna().any():
                ...
                if "Temperature" in selected_param and "Radiator" not in selected_param and "Wall" not in selected_param:
                    orig_cols = [c for c in df.columns if "temperature" in c.lower() and "radiator" not in c.lower() and "wall" not in c.lower()]
                elif "Radiator" in selected_param:
                    orig_cols = [c for c in df.columns if "radiator" in c.lower() and "temperature" in c.lower()]
                elif "Wall" in selected_param:
